...
class ExtensionClient {
  ...
  async requestToken(options = { code: null, silent: true }) {
    const code = options?.code || new URLSearchParams(window.location.search).get('code');
    if (!code) {
      if (!options?.silent) {
  ...
  async login(redirectUrlOverride) {
    const url = new URL(this.authressCustomDomain);
    const randomBytes = (window.crypto || window.msCrypto).getRandomValues(new Uint32Array(16));
